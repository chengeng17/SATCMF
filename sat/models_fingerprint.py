...
#         print(f'self.embedding_edge size: {self.embedding_edge.weight.size()}')
...
#         fp_density_morgan_dim = 3
#         if max_seq_len is None:
#             self.classifier = nn.Sequential(
#                 nn.Linear(d_model , d_model),
#                 nn.ReLU(True),
#                 nn.Linear(d_model + fp_density_morgan_dim, num_class)
#         )
#         else:
#             self.classifier = nn.ModuleList()
#             for i in range(max_seq_len):
#                 self.classifier.append(nn.Linear(d_model + fp_density_morgan_dim, num_class))
#         # if max_seq_len is None:
#         #     self.classifier = nn.Sequential(
#         #         nn.Linear(d_model, d_model),
#         #         nn.ReLU(True),
...
#         fp_density_morgan = data.fp_density_morgan

#         #     # 打印 edge_attr 的类型和形状
#         # print(f'edge_attr type: {edge_attr.dtype}, shape: {edge_attr.shape}')

#         # # 打印 edge_attr 的最小值和最大值
#         # print(f'edge_attr min: {edge_attr.min()}, max: {edge_attr.max()}')
...
#         # 合并分子指纹
#         fp_density_morgan = torch.unsqueeze(fp_density_morgan, dim=-1)  # 为fp_density_morgan添加一个维度
#         output = torch.cat((output, fp_density_morgan), dim=1)  # 现在可以将它们拼接在一起

#         # output = torch.cat((output, fp_density_morgan), dim=1)
...
class GraphTransformer(nn.Module):
    def __init__(self, in_size, num_class, d_model, num_heads=8,
                 dim_feedforward=512, dropout=0.0, num_layers=4,
                 batch_norm=False, abs_pe=False, abs_pe_dim=0,
                 gnn_type="graph", se="gnn", use_edge_attr=True, num_edge_features=15,
                 in_embed=False, edge_embed=False,use_fp_density_morgan=False,use_global_pool=True, max_seq_len=None,
                 global_pool='cls', **kwargs):
        super().__init__()
        ...
        self.use_fp_density_morgan = use_fp_density_morgan  
        
        if use_fp_density_morgan:
            fp_density_morgan_dim = 2048
            self.fc_fp_density_morgan = nn.Linear(fp_density_morgan_dim, 64)

        self.fc_output = nn.Linear(d_model, 64)  
        self.classifier = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.ReLU(True),
    ...
    def forward(self, data, return_attn=False):
        ...
        # print(self.embedding.weight.size(0))
        ...
        if self.use_fp_density_morgan:  # 根据use_fp_density_morgan决定是否使用fp_density_morgan
            fp_density_morgan = data.fp_density_morgan
            fp_density_morgan = self.fc_fp_density_morgan(fp_density_morgan)
            output = output + fp_density_morgan 
