...
class GraphTransformer(nn.Module):
    def __init__(self, in_size, num_class, d_model, num_heads=8,
        ...
        fp_density_morgan_dim = 1048
        # self.fc = nn.Linear(d_model, 64)  # 添加新的全连接层

        self.fc_output = nn.Linear(d_model, 64)  # 用于处理output的全连接层
        self.fc_fp_density_morgan = nn.Linear(fp_density_morgan_dim, 64)  # 用于处理fp_density_morgan的全连接层
        self.classifier = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.ReLU(True),
                nn.Linear(d_model, num_class)
        ...
        # if max_seq_len is None:
        #     self.classifier = nn.Sequential(
        #         nn.Linear(64 + fp_density_morgan_dim, num_class)  # 修改输入维度
        #     )
        # else:
        #     self.classifier = nn.ModuleList()
        #     for i in range(max_seq_len):
        #         self.classifier.append(nn.Linear(64 + fp_density_morgan_dim, num_class))  # 修改输入维度
    ...
    def forward(self, data, return_attn=False):
        ...
        # print(f"Morgan分子指纹的密度: {fp_density_morgan.shape}")
        fp_density_morgan = self.fc_fp_density_morgan(fp_density_morgan)
        
        # print(f"更新后Morgan分子指纹的密度: {fp_density_morgan.shape}")
        output = output + fp_density_morgan  # 将output和fp_density_morgan的输出加在一起
